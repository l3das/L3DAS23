...
def validate_task1_submission(submission_folder, test_folder):
    ...
    def check_subfolder(subfolder):
        contents_submitted = sorted(os.listdir(os.path.join(submission_folder, subfolder)))
        ...
        for i in contents_test:
            submitted_path = os.path.join(submission_folder, subfolder, i.split('.')[0]+'.npy')
    ...
    subfolders_submitted = list(os.listdir(submission_folder))
    if "1mic" not in subfolders_submitted or "2mic" not in subfolders_submitted:
        raise AssertionError ('You should submit results of both 1-mic and 2-mic configs')
    
    check_subfolder("1mic")
    check_subfolder("2mic")
...
def validate_task2_submission(submission_folder, test_folder):
    ...
    def check_subfolder(subfolder):
        ...
        non_csv = [x for x in contents_submitted if x[-4:] != '.csv']  #non .csv files
        if len(non_csv) > 0:
        ...
        for i in contents_submitted:
            submitted_path = os.path.join(submission_folder, subfolder, i)
