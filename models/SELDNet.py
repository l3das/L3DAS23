...
class audiovisual_Seldnet_augmented(nn.Module):
    def __init__(self, time_dim, freq_dim=256, input_channels=4, output_classes=14,
                 pool_size=[[8,2],[8,2],[2,2],[1,1]], cnn_filters=[64,128,256,512], pool_time=True,
                 rnn_size=256, n_rnn=3, fc_size=1024, dropout_perc=0.3, class_overlaps=3.,\
                 pool_size_visual=[2,2,2,2,2], cnn_filters_visual=[32,32,64,128,300],
                 verbose=False):
        super(audiovisual_Seldnet_augmented, self).__init__()
        self.verbose = verbose
        self.time_dim = time_dim
        self.freq_dim = freq_dim
        sed_output_size = output_classes * class_overlaps    #here 3 is the max number of simultaneus sounds from the same class
        ...
        self.cnn_audio = nn.Sequential(*conv_layers)

        self.cnn_visual = nn.Sequential(*conv_layers_visual)
        self.flatten_visual = nn.Flatten(start_dim=2)
    ...
    def forward(self, audios, images):
        
        x_visual = self.cnn_visual(images)
        x = self.cnn_audio(audios)
        if self.verbose:
            print ('cnn out ', x.shape)    #target dim: [batch, n_cnn_filters, 2, time_frames]
        x = x.permute(0,3,1,2) #[batch, time, channels, freq]
        if self.verbose:
        ...
        # print(f"x_visual.shape: {x_visual.shape}")
        x_visual = self.flatten_visual(x_visual)
        # print(f"x_visual.shape after flatten: {x_visual.shape}")
        x_visual = self.visual_output(x_visual)
        # print(f"x_visual.shape after visual_output: {x_visual.shape}")
        audio_visual = torch.concat([x_visual, x], dim=-1)
        # print(f"After concat: {audio_visual.shape}")
        sed = self.sed(audio_visual)
        doa = self.doa(audio_visual)
        if self.verbose:
            print ('sed prediction:  ', sed.shape)  #target dim: [batch, time, sed_output_size]
            print ('doa prediction: ', doa.shape)  #target dim: [batch, time, doa_output_size]
