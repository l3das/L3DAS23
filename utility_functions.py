...
def csv_to_matrix_task2(path, class_dict, dur=60, step=0.1,
    ...
    for index, s in df.iterrows():  #iterate each sound in the list
        ...
        for f in sound_frames:
            ...
            #write loc labels
            loc[f][class_id][pos][0] = s['rho']
            loc[f][class_id][pos][1] = s['theta']
...
def audio_image_csv_to_dict(path):
    '''
    Takes the audio-to-image csv (composed by (name_audio, image_filename) couples).
    Outputs a dict {name_audio: image_filename}
    ''' 
    return pd.read_csv(path, index_col=0, header=None, squeeze=True).to_dict()
...
def load_image(path, resize=None):
    ...
    # image_np = np.empty([3, img.height, img.width], dtype=np.uint8)
    # if error_loading:
    #     image_np = np.asarray(img, dtype=np.float32)
    # else:
    #     img.copyto(image_np)
