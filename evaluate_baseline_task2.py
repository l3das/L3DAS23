...
def main(args):
    ...
    audio_predictors = np.array(predictors[0])
    visual_predictors = np.array(predictors[1])
    target = np.array(target)

    print ('\nShapes:')
    print ('Predictors: ', predictors[0].shape)
    print ('Target: ', target.shape)
    #convert to tensor
    audio_predictors = torch.tensor(audio_predictors).float()
    # visual_predictors = torch.tensor(visual_predictors).float()
    ...
    test_dataset = CustomAudioVisualDataset((audio_predictors, visual_predictors), target, args.path_images, args.path_csv_images, transform)
    dataloader = utils.DataLoader(test_dataset, 1, shuffle=False)

    if not os.path.exists(args.results_path):
        os.makedirs(args.results_path)

    #LOAD MODEL
    n_time_frames = audio_predictors.shape[-1]
